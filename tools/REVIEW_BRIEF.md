...
## How to run it
- Playwright is installed in /private/tmp/claude-501/-Users-mac-mini--src-tennis-player-site/b752de7a-8692-4cd9-a566-ede116a0d8a8/scratchpad (`cd` there; `node -e "require('playwright')"` works). Chromium binaries live in ~/Library/Caches/ms-playwright/ (chromium-1134 and chromium-1228 both present). The default Chromium fails to spawn (error -88). ALWAYS launch with `chromium.launch({ executablePath: '/Users/mac-mini/Library/Caches/ms-playwright/chromium_headless_shell-1228/chrome-headless-shell-mac-arm64/chrome-headless-shell' })` — verified working. There is an existing helper `shoot.js` in that directory you can read and reuse.
